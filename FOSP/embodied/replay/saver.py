...
class Saver:
  ...
  def __init__(self, directory, chunks=1024):
    ...
    # where to load dataset
    self.load_dir = embodied.Path('./data/CarGoal2data')
    self.chunks = chunks
  ...
  def load(self, capacity, length, load_dir):
    if load_dir == False:
      filenames = chunklib.Chunk.scan(self.directory, capacity, length - 1)
    if load_dir == True:
      filenames = chunklib.Chunk.scan(self.load_dir, capacity, length - 1)
    if not filenames:
      return
