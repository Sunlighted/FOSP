...
class Agent(nj.Module):
  ...
  def __init__(self, obs_space, act_space, step, config):
    ...
    self.task_behavior = getattr(behaviors_sd, config.task_behavior)(
        self.wm, self.act_space, self.config, name='task_behavior')
    if config.expl_behavior == 'None':
      self.expl_behavior = self.task_behavior
    elif config.expl_behavior in ['CEMPlanner', 'CCEPlanner', 'PIDPlanner']:
      self.expl_behavior = getattr(behaviors_sd, config.expl_behavior)(
...
class ImagActorCritic(nj.Module):
  ...
  def train(self, imagine, start, context, offline_policy=None):
    def loss(start):
      policy = lambda s: self.actor(sg(s)).sample(seed=nj.rng())
      traj = imagine(policy, start, self.config.imag_horizon)
      loss, metrics = self.loss(traj)
