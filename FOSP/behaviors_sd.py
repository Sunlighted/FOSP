...
class Greedy(nj.Module):
  ...
  def __init__(self, wm, act_space, config):
    ...
    if config.critic_type == 'vfunction':
      critics = {'extr': agent_sd.VFunction(rewfn, config, name='critic')}
      if config.use_cost:
        cost_critics = {'extr': agent_sd.CostVFunction(costfn, config, name='cost_critic')}
    else:
      raise NotImplementedError(config.critic_type)

    if config.use_cost:
      self.ac = agent_sd.ImagSafeActorCritic(
          critics, cost_critics, {'extr': 1.0}, {'extr': 1.0}, act_space, config, name='safe_ac')
    else:
      self.ac = agent_sd.ImagActorCritic(
...
class Explore(nj.Module):
  ...
  def __init__(self, wm, act_space, config):
    ...
    for key, scale in config.expl_rewards.items():
      ...
      if key == 'extr':
        rewfn = lambda s: wm.heads['reward'](s).mean()[1:]
        critics[key] = agent_sd.VFunction(rewfn, config, name=key)
      else:
