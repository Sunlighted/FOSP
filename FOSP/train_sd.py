...
def main(argv=None):
  ...
  logdir_algo = config.logdir + now_time + '_' + str(config.method) + '_' + str(config.task) + '_' + str(config.seed)  + 'collect'
  ...
  try:
    ...
    if args.script == 'train':
    ...
    elif args.script == 'train_eval_offline':
      replay = make_replay(config, logdir / 'replay')
      ...
      agent = agt.Agent(env.obs_space, env.act_space, step, config)
      embodied.run.train_eval_offline(
          agent, env, eval_env, replay, eval_replay, logger, args, lag)
      
    elif args.script == 'train_eval_online':
      replay = make_replay(config, logdir / 'replay')
      eval_replay = make_replay(config, logdir / 'eval_replay', is_eval=True)
      env = make_envs(config)
      eval_env = make_envs(config)  # mode='eval'
      cleanup += [env, eval_env]
      agent = agt.Agent(env.obs_space, env.act_space, step, config)
      embodied.run.train_eval_online(
          agent, env, eval_env, replay, eval_replay, logger, args, lag)
